...
# 根据指定的Username发消息
#def sendMsg(MyUserName, ToUserName, msg, seconds):           #自己注释
def sendMsg(MyUserName, ToUserName, msg):                   #自己加入
...
def sendInterface(MemberList, MemberCount):
    ...
    return thread_res 
def sendInterface1(MemberList, MemberCount,msg):             #自己加入的，用来全部群发，新加参数msg用来群发
    thread_res = [] 
    for i in range(MemberCount):
        to_user_found = MemberList[i]['UserName']
##        if(flag == '1'):
##            mark_name = MemberList[i]['RemarkName']
##            t = threading.Thread(target = sendMsg, args = (My['UserName'], to_user_found, mark_name+' '+msg))
##        else:
##            t = threading.Thread(target = sendMsg, args = (My['UserName'], to_user_found, msg))
        t = threading.Thread(target = sendMsg, args = (My['UserName'], to_user_found, msg))
        thread_res.append(t)
    return thread_res
def main1():
...
def main2(pp):
    ...
    threads = []
##    while(True):                                          while到break全部自己注释
##        #进入发送界面
##
##        threads_res = sendInterface(MemberList, MemberCount)      
##        threads_res = sendInterface1(MemberList, MemberCount,'你好')      
...
##            break
    ...
    threads_res = sendInterface1(MemberList, MemberCount,pp)    #从这到***处自己添加    
    threads_num = len(threads_res)
    for i in range(threads_num):
        threads.append(threads_res[i])                      #***
